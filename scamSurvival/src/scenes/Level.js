...
class Level extends Phaser.Scene {
	...
	editorCreate() {
		...
		// pharmacy
		const pharmacy = this.add.image(388, 304, "bg");
		pharmacy.scaleX = 1.1;
		pharmacy.scaleY = 1.25;
		pharmacy.visible = false;

		// pharmaPaper_1
		const pharmaPaper_1 = this.physics.add.sprite(400, 501, "shako");
		...
		// pharmaPaper
		const pharmaPaper = this.physics.add.sprite(400, 501, "shako");
		...
		// calc
		const calc = this.physics.add.sprite(1559, -145, "calculator");
		calc.body.setSize(285, 385, false);
	...
	generateIkea(y,name, maxX) {			
		...
		this[name] = this.physics.add.sprite(randomX, y, "calculator");
		this[name].scaleX =  0.3;
		this[name].scaleY = 0.3;
	...
	generateOmeprazol(y,name) {
		var collideCallback = function(gameObject1, gameObject2) {
			if (gameObject2.height==606) {
				this.omeprazols=this.omeprazols+10;
			}
			if (gameObject2.height==314) {
				this.omeprazols=this.omeprazols+20;
			}		
			if (gameObject2.height==575) {
				this.omeprazols=this.omeprazols+100;
			}					
			this.textOme.text="Money: "+this.omeprazols;
			gameObject2.destroy();
		}
		...
		if (sprite==0) {
			this[name] = this.physics.add.sprite(randomX, y, "rand10");
			this[name].scaleX =0.1;
			this[name].scaleY = 0.1;
		}
		if (sprite==1) {
			this[name] = this.physics.add.sprite(randomX, y, "rand20");
			this[name].scaleX =0.2;
			this[name].scaleY = 0.2;
		}
		if (sprite==2) {
			this[name] = this.physics.add.sprite(randomX, y, "rand100");
			this[name].scaleX =0.1;
			this[name].scaleY = 0.1;	
		}
